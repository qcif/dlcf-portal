...
do
    ...
    if [ $var = "compile" ]; then
      docker run -it --rm -v $PWD:/opt/rds-rdmp-portal qcifengineering/dlcf-portal:latest /bin/bash -c "cd /opt/rds-rdmp-portal && npm install grunt-cli -g && grunt --gruntfile Gruntfile-ts-compile.js"
    fi
done
